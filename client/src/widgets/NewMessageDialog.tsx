...
const NewMessageDialog = ({ userId, username, avatarUrl }: NewMessageDialogProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = await createDirectChat({ participantId: userId, content: newMessage });
     ...
     <Dialog>
      <form onSubmit={handleSubmit} id={`new-message-form-${userId}`} >
        <DialogTrigger asChild>
          <Button variant="link">New Message</Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button type="submit" form={`new-message-form-${userId}`}>Send</Button>
          </DialogFooter>
        </DialogContent>
      </form>
    </Dialog>
